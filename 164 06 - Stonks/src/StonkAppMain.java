...
public class StonkAppMain {
    ...
    /*
     * The go method is the main start function for the program.
     * The go starts the user interface and prompts as well as
     * checks that the user does not enter "x" to stop the program.
     */
    public void go(){
        view.startScreen();
        walletInitializer();
    ...
    public void purchaseFazCoin(int numFazCoin){
        double pricePerCoin = sm.getExchangeRate();
        double totalCost = pricePerCoin * numFazCoin;
        if (w.getUSDollars() >= totalCost) {
            w.setFazCoin(numFazCoin);
            w.setUSDollars(totalCost);
        } else {
    ...
    public void sellFazCoin(int numFazCoin){
        if (w.getFazCoin() >= numFazCoin) {
            double pricePerCoin = sm.getExchangeRate();
            double totalEarnings = pricePerCoin * numFazCoin;
            w.setFazCoin(numFazCoin);
            w.setUSDollars(totalEarnings);
        } else {
