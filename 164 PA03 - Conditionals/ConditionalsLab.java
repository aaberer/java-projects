...
public class ConditionalsLab  {
    ...
    * Lane check takes an integer input and runs it through the 
    * conditional if statements to catergorize the speed into different
    * catagories and returns a string value for said catagory
    */
     ...
     * Had to change the conditional statement to disregard the changes
     * in case sensitivity
     */
    ...
    /* Tree Diagram
     * When input is go light2A checks for any change in the color
     * and gives instrution for the car/driver
     * 
     * Testing
     * Changing the .equalsIgnoreCase to .contains didn't change
     * any of my tests for the application
     */
    ...
    public static void testStreetLight2AB(){
        ...
        String test1A = streetLight2A(new String("red"));
        String test2A = streetLight2A(new String("yellow"));
        String test3A = streetLight2A(new String("green"));

        System.out.printf("Test1 should return Stop: %b (%s)%n", 
            "Stop".equals(test1A), test1A);
        System.out.printf("Test2 should return Slow: %b (%s)%n", 
            "Slow".equals(test2A), test2A);
        System.out.printf("Test3 should return Go: %b (%s)%n", 
            "Go".equals(test3A), test3A);

        System.out.println("Testing streetLight2B");
        String test1B = streetLight2A(new String("red"));
        String test2B = streetLight2A(new String("yellow"));
        String test3B = streetLight2A(new String("green"));

        System.out.printf("Test1 should return Stop: %b (%s)%n", 
            "Stop".equals(test1B), test1B);
        System.out.printf("Test2 should return Slow: %b (%s)%n", 
            "Slow".equals(test2B), test2B);
        System.out.printf("Test3 should return Go: %b (%s)%n", 
            "Go".equals(test3B), test3B);
    }
